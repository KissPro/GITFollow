...
namespace Test_service.Helper
{
    public class APIHelper
    {
        ...
        public APIHelper()
        {
            proxy = Proxy.GetProxyForUrlUsingPac(ConfigurationManager.AppSettings["getRequestsPath"], "http://hnproxy.fushan.fihnbb.com/proxy.pac");
            var proxiedHttpClientHandler = new HttpClientHandler() { UseProxy = true };
            proxiedHttpClientHandler.Proxy = new WebProxy(proxy);
            proxiedHttpClientHandler.Proxy.Credentials = new NetworkCredential("V1517299", "Saodoingoi!2", "FUSHAN");
            HttpClient = new HttpClient(proxiedHttpClientHandler);
            HttpClient.BaseAddress = new Uri(ConfigurationManager.AppSettings["getRequestsPath"]);
        }
    }
